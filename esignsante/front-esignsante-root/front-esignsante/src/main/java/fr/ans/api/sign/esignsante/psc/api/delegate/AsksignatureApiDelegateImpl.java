...
public class AsksignatureApiDelegateImpl extends AbstractApiDelegate implements AsksignatureApiDelegate {
	...
	@Override
	    public ResponseEntity<org.springframework.core.io.Resource> postAskSignaturePades(String xUserinfo, String xIntrospectionResponse,
	    		 String xTokenPSC, String xTokenAPI, MultipartFile file) {
		...
		ParametresSign params = prepareAppelEsignWSPSCDatas (xUserinfo, xIntrospectionResponse , xTokenPSC, xTokenAPI);
 ...
 */
	@Override
	public ResponseEntity<String> postAsksignatureXades(String xUserinfo, String xIntrospectionResponse, String xTokenPSC,
			String xTokenAPI, MultipartFile file) { 
	...
	@Override
	public ResponseEntity<org.springframework.core.io.Resource> postAskSignatureFse(String xUserinfo, String xIntrospectionResponse, byte[] hashFSE, 
			String idFacturationPS,String xTokenPSC, String xTokenAPI, String typeFlux) { 
	...
	private UserInfo decodeBase64UserInfoJSON(String jsonUserInfoBase64) {
		...
		checkNotEmptyInputParameters(params.getTokenPSC(), params.getTokenAPI(),file, params.getJsonUserInfo(), params.getJsonPscReponse());
	...
	public void checkNotEmptyInputParameters(String tokenPSC, String tokenAPI, MultipartFile file, String userinfo, String tokenValidationResponse) {
		if ((tokenPSC == null) || (tokenAPI == null) ||  (file == null) || (userinfo == null) || (tokenValidationResponse == null)) {
			throwExceptionRequestError(
					"Au moins un paramètre indispensable parmi {'file', 'userinfo', 'tokenPSC', 'tokenAPI', 'tokenValidationResponse'} est nul ou non fourni",
					HttpStatus.BAD_REQUEST);
		}

		if ((tokenPSC.isEmpty()) || (tokenPSC.isBlank()) ||
				(tokenAPI.isEmpty()) || (tokenAPI.isBlank()) || 
				(file.getSize() <= 0) || (userinfo.isEmpty())
				|| (userinfo.isBlank()) || (tokenValidationResponse.isEmpty())	|| (tokenValidationResponse.isBlank()) ) {
			throwExceptionRequestError("Au moins un paramètre parmi {'file', 'userinfo', 'tokenPSC', 'tokenAPI', 'tokenValidationResponse'} est vide",
					HttpStatus.BAD_REQUEST);
		}
	}
	
	private ParametresSign prepareAppelEsignWSPSCDatas (final String xUserInfo, final String xIntrospectionResponse,
			final String xTokenPSC, final String xTokenAPI) {
		
		ParametresSign params = new ParametresSign();
		String decodedIntro = null;
		String decodedUserInfo = null;
		
		params.setTokenPSC(xTokenPSC);
		params.setTokenAPI(xTokenAPI);
	...
	openidToken.setAccessToken(params.getTokenPSC());
	openidToken.setApiToken(params.getTokenAPI());
...
//	var openidTokenAPI = new OpenidToken();
//	openidToken.setAccessToken(params.getTokenAPI());
//	openidTokens.add(openidTokenAPI);
	return openidTokens;
	}
