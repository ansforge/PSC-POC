job "keycloak-server" {
    ...
    group "keycloak-server" {
        ...
        task "keycloak-server" {
			...
			artifact {
				source = "https://github.com/prosanteconnect/proof-of-concept/raw/main/keycloak/keycloak-server/truststore.pem"
			}
            ...
            config {
				...
				mount {
					type = "bind"
					target = "/opt/bitnami/keycloak/certs/truststore.pem"
					source = "local/truststore.pem"
					readonly = "false"
					bind_options {
						propagation = "rshared"
					}
...
EOF
			...
			template {
				data = <<EOH
					...
					KEYCLOAK_HTTPS_TRUST_STORE_FILE = /opt/bitnami/keycloak/certs/truststore.pem
